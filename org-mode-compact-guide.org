...
- {{{kbd(S-SU)}}} (~org-priority-up~); {{{kbd(S-GIU)}}} (~org-priority-down~) ::
  ...
  Aumentano/diminuiscono la piorità dell'intestazione corrente.
...
ad albero con sottoelementi TODO, con dettagliati i sottocompiti
...
una intestazione. Dopo i duepunti, {{{kbd(M-TAB)}}} offre il completamento dei
...
segnati con almeno uno dei membri, del gruppo o ogniuno dei sottogruppi.
...
- {{{kbd(C-c C-x C-j)}}} (~org-clock-goto~) ::
  ...
  Salta all'intestazionv del lavoro, nel corrente timer in lavoro. Con un argomento
...
- {{{kbd(C-c C-c C-w)}}} (~org-agenda-refile~) ::
  ...
  Usa l'interfaccia di spostameto per saltare ad una intestazione.
...
- {{{kbd(m)}}}, {{{kbd(M)}}} ::
  ...
  Crea una lista di intastazioni che corrispondono all'espressione data ( vedi
...
- {{{kbd(M-x org-agenda M)}}}(~org-tags-view~) ::
  ...
  Come {{{kbd(m)}}} ma seleziona solo le intestazion che sono oggetti TODO.
...
- {{{kbd(u)}}} ::
  ...
  Come per la ricerca globale di tags per le intestazion segnate =boss= ma
  non =urgent=.
...
Inserimento di un blocco di esportazione nel file Org, dove il beckend
...
duepunti iniziale seguito dal nome dall'argomento in minuscolo.
