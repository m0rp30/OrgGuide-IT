...
#+begin_quote
 ...
 per favore salta questa sezione e vai direttamente all'[[*Attivazione]].
#+end_quote
...
sui bug, guarda il [[info:manual][[[info:org][manuale principale]].
...
nascondendo le funzionalità con un singolo comando ~org-cycle~ che é
...
asterisco. Guarda nelle [[*Miscellanee]] per un setup che lo realizza.
...
giusto due comandi, legati a {{{kbd(TAB)}}} e {{{kbd(S-TAB)}}} (~org-cycle~ e
~org-shifttab~ ) per scambiare la visibilità nel buffer.
...
- {{{kbd(C-c C-n)}}} (~org-next-visible-heading~) :: Intestazione successivo
- {{{kbd(C-c C-p)}}} (~org-previous-visible-heading~) :: Intestazione precedente

- {{{kbd(C-c C-f)}}} (~org-backward-headling-same-level~) :: Intestazione successivo nello stesso livello
...
- {{{kbd(M-SU)}}} (~org-move-subtree-up~), {{{kbd(M-GIÙ)}}} (~org-move-subtree-down~) ::
  ...
  Muove il sotto-albero su o giù es. scambia con il precedente o il
...
- {{{kbd(M-S-SU)}}} (~org-move-item-up~), {{{kbd(M-S-GIU)}}} (~org-move-item-down~) ::
  ...
  Muove l'elemento, inclusi i sotto-elementi, su/giù ( spostandoli con
...
- {{{kbd(C-c C-c)}}} (~org-toggle-checkbox~) ::
  ...
  Se c'è una casella di controllo ( vedere [[*Liste selezionabili]] ) nella linea
...
un nuovo campo con {{{kbd(TAB)}}}, {{{kbd(S-TAB)}}} o {{{kbd(RET)}}}/,
...
- {{{kbd(C-c RET)}}} (~org-table-hline-and-move~) ::
  ...
  Inserisce una linea sotto la corrente e muove il punto nella riga
  sotto questa linea.
...
identificazione seguita dai due-punti *:*. Non ci sono spazzi dopo i
...
- {{{kbd(C-c C-l)}}} (~org-insert-link~) ::
  ...
  Quando chiamato con l'argomento prefisso {{{kbd(C-u)}}} il
...
possono dare priorità mettendo un /priority cookie/ nell'intestazione
...
- {{{kbd(C-c \,)}}} (~org-priority~) ::
  ...
  Imposta la priorità nell'intestazione corrente. Premi {{{kbd(A)}}},
...
- {{{kbd(M-S-RET)}}} (~org-insert-todo-heading~) ::
  ...
  se ci si trova in un elemento della lista ( vedi [[*Liste semplici]] ).
...
informazioni incrociate correlate, é quello di assegnare dei /tag/ alle
...
due-punti, es. =:work:=. Alcuni tag possono essere specificati come in
...
una intestazione. Dopo i due-punti, {{{kbd(M-TAB)}}} offre il completamento dei
...
Un tag può essere definito come un /gruppo di tag/ per un insieme di
...
- {{{kbd(C-c / m)}}} o {{{kbd(C-c \)}}} (~ org-match-sparse-tree~) ::
  ...
  Crea un albero sparso con tutte le intestazioni che corrispondono ad
  un tag cercato. Con un argomento prefisso {{{kbd(C-u)}}}, ignora le
...
un elemento in una specifica data nell'agenda ( vedi [[L'agenda
Settimanale/Giornaliera]] ). Distinguiamoli:

- Timestamp semplici; Eventi; Appuntamenti ::

  Un semplice timestamp che assegna ad un elemento una
...
selezionare una data nel calendario pop-up. Consulta il [[info:org][manuale]] per
...
- {{{kbd(C-c C-d)}}} (~org-deadline~) ::
  ...
  la mancata scadenza, a partire da ~org-deadline-warning-days~ prima
...
- {{{kbd(C-c C-s)}}} (~org-schedule~) ::
  ...
  L'intestazione é mostrata nella data[fn:8] impostata. In aggiunta, nella
...
così detto, repeater in una =DEADLINE=, =SCHEDULED= o in un timestamp
...
#+texinfo: @noindent
il =+1m= é un repeater; quello che si intende é che il
...
- {{{kbd(C-c C-x C-j)}}} (~org-clock-goto~) ::
  ...
  Salta all'intestazione del lavoro, nel corrente timer in lavoro. Con un argomento
...
[[L'agenda Settimanale/Giornaliera]] ), mostra su quali task stai
...
L'impostazione seguente setta un file target[fn:9] di default per le
note.
...
- {{{kbd(C-c C-w)}}} (~org-capture-refile~) ::
  ...
  differente posizione ( vedi [[Sposta e Copia]] ).
...
=journal.org=. Una possibile configurazione potrebbe essere simile a
questa:
...
% [fn:10] permette l'inserimento dinamico del contenuto. Qui c'è una
piccola selezione di possibilità, consulta il [[info:manual][manuale]] per averne di
più.

| =%a=     | annotazione, di norma il link creato con ~org-store-link~                 |
...
- {{{kbd(C-c C-x C-s)}}} o brevemente {{{kbd(C-c $)}}} (~org-archive-subtree~) ::
  ...
  Archivia il sotto-albero partendo dalla posizione del punto nella
...
{{{kbd(M-x org-agenda)}}}, o, meglio se abbinato ad una chiave globale (
...
- {{{kbd(m)}}}, {{{kbd(M)}}} ::
  ...
  Crea una lista di intestazioni che corrispondono all'espressione data ( vedi
...
- {{{kbd(M-x org-agenda a)}}} (~org-agenda-list~) ::
  ...
  Compila una agenda per la settimana corrente da una lista di file
...
- {{{kbd(M-x org-agenda t)}}} (~org-todo-list~) ::
  ...
  Mostra la lista globale dei TODO. Questo raggruppa gli elementi TODO
...
[[*TAG]] ), o hanno delle proprietà ( vedi [[*Proprietà]] ), puoi selezionare le
...
Una stringa di ricerca può usare operatori Booleani =&= per AND e =|=
...
- =+work-boss= ::
  ...
  Seleziona le intestazioni taggate con =work=, ma scarta quelle che sono
  marcate =boss=
...
[[info:manul][manuale]] per informazioni aggiuntive.
...
Per esempio, la stringa di ricerca =computer equipment= trova gli
...
corrispondono all'espressione regolare =8\.11[bg]=. che esclude sia
=8.11b= che =8.11g=.
...
- {{{kbd(C-k)}}} (~org-agenda-kill~) ::
  ...
  Cancella l'oggetto corrente dall'agenda e lungo l'intero sotto-albero
...
- {{{kbd(a)}}} (~org-agenda-archive-default-with-confirmation~) ::
  ...
  Archivia il sotto-albero corrispondente all'elemento del punto usando
...
Distributore Agenda]] ). Puoi anche impostarla direttamente con Emacs
...
- {{{kbd(u)}}} ::
  ...
  Come per la ricerca globale di tags per le intestazioni segnate =boss= ma
  non =urgent=.

- {{{kbd(v)}}} ::

  La stessa ricerca, ma limitata per le intestazioni che sono anche elementi
  TODO.
...
assieme al framework di esportazione ( vedi [[*Esportazione]] ), con Org
...
intero, imposta la variabile ~org-export-with-toc~. Puoi fare la
stessa cosa per ogni file, usando i seguenti elementi =toc= nella
parola chiave =OPTIONS=:
...
o =src=. Il terzo parametro, anch'esso opzionale, specifica il
...
di configurare una conversione HTML automatica dei /progetti/ composta
...
[[info:manual]]. Qui c'è un esempio:
...
blocco di codice, pronto per la modifica. Usando {{{kbd(C-c ')}}}
...
- Argomenti di intestazione a livello di sistema ::
  ...
  Questi sono specificati personalizzando la variabile
...
/formato/ e la /manipolazione/ dei risultati dei blocchi di codice.
...
L'argomento d'intestazione =exports= serve ad informare che quella
...
Org ha il completamento nel buffer con {{{kbd(M-TAB)}}}. Il mini-buffer
...
( con {{{kbd(TAB)}}} ) il testo sotto ad ogni suo titolo, a nascondere
...
C-c)}}} si attivano i cambiamenti nella linea.
