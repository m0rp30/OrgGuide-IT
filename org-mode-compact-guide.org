...
Questo documento é un derivato molto compresso dal più completo [[https://www.gnu.org/software/emacs/manual/html_mono/org.html#Top][manuale Org
...
#+begin_quote
 ...
 perfavore salta questa sezione e vai direttamente a [[*Attivazione]].
#+end_quote

Se hai scaricato Org dal web, solitamente viene distribuito come file =.zip=
o =.tar= oppure come archivio Git, é meglio avviarlo direttamente dalla
directory di distribuzione. Hai bisogno di aggiungere la sottodirectory
=lips/= alla path di Emacs. Per fare questo aggiungi la seguente linea nel
...
~org-cycle~ che é legato al tasto {{{kbd(TAB)}}}.
...
Nota che un'intestazione nominata dopo ~org-footnote-section~, che di default
é detto =Footnotes=, é considerato come speciale. Un sottoalbero con questa
...
nelle [[*Miscelanee]] per un setup che lo realizza.
...
- {{{kbd(C-u C-u C-u TAB)}}} (~org-show-all~) ::
  ...
  ~org-startup-folder~ o nell'intestazione del file aggiungendo una keyword
  =STARTUP= con =overview=, =content=, =shoall=, =showeverything= o =show<n>levels= (n
  = 2..5 ) tipo:
...
- {{{kbd(C-c C-n)}}} (~org-next-visible-heading~) :: Titolo sucessivo
- {{{kbd(C-c C-p)}}} (~org-previous-visible-heading~) :: Titolo precedente

- {{{kbd(C-c C-f)}}} (~org-backward-headling-same-level~) :: Titolo sucessivo nello stesso livello

- {{{kbd(C-c C-b)}}} (~outline-backward-same-level~) :: Titolo precedente nello stesso livello

- {{{kbd(C-c C-u)}}} (~outline-up-heading~) :: Torna al titolo più altro
...
- *{{{kbd(M-RET)}}}* (~org-meta-return~) ::
  ...
  Inserisce un nuovo titolo con lo stesso livello del corrente. Se il punto
  é una lista di oggetti, viene creato un nuovo oggetto (vedere [[Liste
  semplici]]). Quando questo comando viene usato nel mezzo di una linea, la
  linea viene divisa e il resto della linea diventa il nuovo titolo[fn:2]

- {{{kbd(M-S-RET)}}} (~org-inser-todo-heading~) ::

  Inserisce un nuovo oggetto TODO con lo stesso livello del titolo corrente
...
- {{{kbd(M-SINISTRA)}}} (~org-metaleft~), {{{kbd(M-DESTRA)}}} (~org-metaright~) ::
  ...
  Promuove o retrocede il titolo corrente di un livello
...
- {{{kbd(C-c -)}}} (~org-cycle-list-bullet~) ::
  ...
  Scorre tra la lisata dei livelli degli elementi attraverso
...
con =|= come primo carattere senza spazi viene considerato come parte
di una tabella. Il carattere =|= é anche un separatore di colonne. Una
...
=[[#my-custom-id]]= che viene collegata ad un elemento con la
proprietà =CUSTOM_ID= settata a =my-custom-id=.

Link tipo =[[My Target]]= o =[[My Targhet][Find my target]]= va, nel
...
indentificazione se quita dai duepunti *:*. Non ci sono spazzi dopo i
...
- {{{kbd(C-c C-l)}}} (Che punta ad un link esistente) (~org-insert-link~) ::
  ...
  Quando il punto é un link esistente, {{{kbd(C-c C-l)}}} permette la
...
docstring dell'opzinoe ~org-todo-state-tags-triggers~ per dettagli.
...
registrata sotto l'oggetto con un titolo =Closing Note=.
